...
JsonBox::Object JSONDB::query(JsonBox::Object request, unsigned retries)
{
	...
	if (table.length() == 0) {
		...
		return response;
	} else if (table.compare("sip_buddies") == 0 || table.compare("dialdata_table") == 0 ||
		   table.compare("rates") == 0 || table.compare("subscribers") == 0) {
	...
	if (table.compare("subscribers") == 0) {
		if (action.compare("create") == 0) {
			...
			queryTMP << " values (\"" << imsi << "\",\"" << name << "\",\"" << msisdn << "\",\"" << ki
				 << "\",\"dynamic\",\"gsm\",\"127.0.0.1\")";
			...
			q << "select sip_buddies.username as \"imsi\", sip_buddies.name as \"name\", "
			     "dialdata_table.exten as \"msisdn\" from sip_buddies left outer join dialdata_table on "
			     "sip_buddies.username = dialdata_table.dial";
			return read(q.str());
			...
			queryTMP << "update sip_buddies set name=\"" << name << "\", callerid=\"" << msisdn
				 << "\" where username=\"" << imsi << "\"";
			responseSIP = execOnly(queryTMP.str());
